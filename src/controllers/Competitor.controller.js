...
ParticipanteCtrl.createCompetitor = async(req, res) => {
    try {
        ...
        if(data.name === '' || data.id_sede === '' ||  data.address === ''||  data.email === '' || 
            data.url_video === '' ||  data.pay === '' ||  data.personal_identification === ''  || 
            data.phone  === '' ||  data.name_song === '' ||  data.experience === '' ||data.photo === '' ||
            data.place_from  === '' ||  data.birthday === '' || data.genre === '' || data.curp === '')  
        {  
       ...
       console.log({existCurp, existEmail});
        if(existCurp.length > 0 ) {
            res.status(400).json({resp: 'error',message:'Ya se registró un participante con esta CURP.' });
            return;
        }
        if( existEmail.length > 0) {
            res.status(400).json({resp: 'error',message:'Ya se registró un participante con este email.' });
            return;
        }
...
ParticipanteCtrl.queryParticipantes = async(req, res) => {
    try {
      ...
      let OR = [];
      let AND = [];
      if(req.params.competitor){
        OR.push({ name: { $regex: ".*" + req.params.competitor + ".*", $options: "i" } })
      }
      ...
      if(req.params.competitor){
        aggregateFind = await modelCompetitor.aggregate([
            {
                $match: {
                    $or: OR,
        ...
        if(req.params.id_sede){
            aggregateFind = await modelCompetitor.find({ id_sede:req.params.id_sede }); 
        }else{
            aggregateFind = await modelCompetitor.find();
        }
       
      }
         
        const competitor = await modelCompetitor.populate(aggregateFind, {
            path: "id_sede",
        });
